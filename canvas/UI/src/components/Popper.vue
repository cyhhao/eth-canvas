    ...
    export default {
        ...
        methods: {
            ...
            async buy() {
                ...
                try {
                    let ok = await window.buyContract.methods.buyPoint(this.data.x, this.data.y, this.selectedColor)
                        .send({from: window.account, value: this.data.buyPrice[0], gas: 200000})
                    console.log("ok:", ok)
                    if (ok) {
                        this.$Notice.success({
                            title: '购买成功',
